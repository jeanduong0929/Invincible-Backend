          ...
          if (userService.isUniqueEmail(email)) {
            if (userService.isValidPassword(pwd1)) {
              if (userService.isSamePassword(pwd1, pwd2)) {
                ...
                if (role.isEmpty())
                  throw new RoleNotFoundException(
                      "Creating new user failed because DEFAULT role is not established");
              ...
              throw new InvalidRegisterException(
                  "Password needs to be minimum 8 characters, at least 1 letter and 1 number");
          } else
            throw new InvalidRegisterException("Email is already in used");
        } else
          throw new InvalidRegisterException("The email provided is invalid");
      } else
